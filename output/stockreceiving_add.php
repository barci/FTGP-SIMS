...
add_nocache_headers();
include("include/stockreceiving_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["stockreceiving_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "stockreceiving_inline_add.htm";
else
	$templatefile = "stockreceiving_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: stockreceiving_add.php');
...
//	processing SupplierID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_SupplierID = $pageObject->getControl("SupplierID", $id);
		$control_SupplierID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing SupplierID - end
//	processing ProductID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ProductID = $pageObject->getControl("ProductID", $id);
		$control_ProductID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ProductID - end
...
//	processing ReceiveFlag - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ReceiveFlag = $pageObject->getControl("ReceiveFlag", $id);
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="product")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["ProductID"]==""){
			$avalues["ProductID"] = prepare_for_db("ProductID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing SupplierID - start
			...
			if($inlineAddOption)
			{
				$control_SupplierID->afterSuccessfulSave();
			}
//	processing SupplierID - end
//	processing ProductID - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_ProductID->afterSuccessfulSave();
			}
//	processing ProductID - end
//	processing Qty - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'stockreceiving_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'stockreceiving_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: stockreceiving_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="product")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["ProductID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["RequestID"]=@$avalues["RequestID"];
	$defvalues["StaffID"]=@$avalues["StaffID"];
	$defvalues["SupplierID"]=@$avalues["SupplierID"];
	$defvalues["ProductID"]=@$avalues["ProductID"];
	$defvalues["Qty"]=@$avalues["Qty"];
	$defvalues["Notes"]=@$avalues["Notes"];
	$defvalues["ReceiveFlag"]=@$avalues["ReceiveFlag"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("RequestID_label","<label for=\"".GetInputElementId("RequestID", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("SupplierID_label","<label for=\"".GetInputElementId("SupplierID", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("ProductID"))
		$xt->assign("ProductID_fieldblock",true);
	else
		$xt->assign("ProductID_tabfieldblock",true);
	$xt->assign("ProductID_label",true);
	if(isEnableSection508())
		$xt->assign_section("ProductID_label","<label for=\"".GetInputElementId("ProductID", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("ReceiveFlag"))
		$xt->assign("ReceiveFlag_fieldblock",true);
	else
		$xt->assign("ReceiveFlag_tabfieldblock",true);
	$xt->assign("ReceiveFlag_label",true);
	if(isEnableSection508())
		$xt->assign_section("ReceiveFlag_label","<label for=\"".GetInputElementId("ReceiveFlag", $id, PAGE_ADD)."\">","</label>");
...
//	RequestID
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("RequestID", $data, $keylink);
		$showValues["RequestID"] = $value;
...
//	SupplierID
	...
	{	
		...
		$showValues["SupplierID"] = $value;
		$showFields[] = "SupplierID";
	}	
//	ProductID
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ProductID", $data, $keylink);
...
//	ReceiveFlag
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ReceiveFlag", $data, $keylink);
		$showValues["ReceiveFlag"] = $value;
		$showFields[] = "ReceiveFlag";
	}	
		$showRawValues["ID"] = substr($data["ID"],0,100);
		$showRawValues["RequestID"] = substr($data["RequestID"],0,100);
		...
		$showRawValues["ReceiveFlag"] = substr($data["ReceiveFlag"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "stockreceiving";
		$options['firstTime'] = 1;
