...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["orderentry_detailspreview"] = $layout;
...
if($rowcount) {
	...
	//	ODate - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ODate", $data, $keylink);
			$row["ODate_value"] = $value;
	//	ErrorMsg - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ErrorMsg", $data, $keylink);
			$row["ErrorMsg_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("orderentry_detailspreview.htm");
