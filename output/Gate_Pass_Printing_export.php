...
$layout->blocks["top"][] = "export";$page_layouts["Gate_Pass_Printing_export"] = $layout;
...
$xt->display("Gate_Pass_Printing_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Gate_Pass_Printing.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Gate_Pass_Printing.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Gate_Pass_Printing.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["DrNo"] = $pageObject->showDBValue("DrNo", $row);
			...
			$values["subtotal"] = $pageObject->showDBValue("subtotal", $row);
			$values["eta"] = $pageObject->showDBValue("eta", $row);
			$values["delFlag"] = $pageObject->showDBValue("delFlag", $row);
			$values["Destination"] = $pageObject->showDBValue("Destination", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Gate_Pass_Printing.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["DrNo"] = $pageObject->getViewControl("DrNo")->showDBValue($row, "");
			...
			$values["subtotal"] = $pageObject->getViewControl("subtotal")->showDBValue($row, "");
			$values["eta"] = $pageObject->getViewControl("eta")->showDBValue($row, "");
			$values["delFlag"] = $pageObject->getViewControl("delFlag")->showDBValue($row, "");
			$values["Destination"] = $pageObject->getViewControl("Destination")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DrNo"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["subtotal"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Destination"]).'"';
			echo $outstr;
		}
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["orderID"]);
				else
					echo $values["orderID"];//echo htmlspecialchars($values["orderID"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["prodID"]);
				else
					echo $values["prodID"];//echo htmlspecialchars($values["prodID"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["subtotal"]);
					else
						echo $values["subtotal"];
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["delFlag"]);
					else
						echo $values["delFlag"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Destination"]);
					else
						echo $values["Destination"];
			echo '</td>';
			echo "</tr>";
		}
