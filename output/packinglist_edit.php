...
$layout->blocks["top"][] = "details";$page_layouts["packinglist_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "packinglist_inline_edit.htm" : "packinglist_edit.htm";
...
//	processing delNo - begin
	$condition = 1;

	if($condition)
	{
		$control_delNo = $pageObject->getControl("delNo", $id);
		$control_delNo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing delNo - end
//	processing orderNo - begin
	$condition = 1;

	if($condition)
	{
		$control_orderNo = $pageObject->getControl("orderNo", $id);
		$control_orderNo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing orderNo - end
//	processing delID - begin
	$condition = 1;

	if($condition)
	{
		$control_delID = $pageObject->getControl("delID", $id);
		$control_delID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing qty - end
//	processing unit - begin
	$condition = 1;

	if($condition)
	{
		$control_unit = $pageObject->getControl("unit", $id);
		$control_unit->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: packinglist_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: packinglist_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["delNo"] = $evalues["delNo"];
	$data["orderNo"] = $evalues["orderNo"];
	$data["delID"] = $evalues["delID"];
	$data["notes"] = $evalues["notes"];
	$data["qty"] = $evalues["qty"];
	$data["unit"] = $evalues["unit"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("delNo_label","<label for=\"".GetInputElementId("delNo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("orderNo_label","<label for=\"".GetInputElementId("orderNo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("delID_label","<label for=\"".GetInputElementId("delID", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("unit_label","<label for=\"".GetInputElementId("unit", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='packinglist_view.php?".$viewlink."'\"");
