...
class LoginPage extends RunnerPage 
{
	...
	*/
	function LogIn($pUsername,$pPassword){
		...
		$fieldList = "";
		$lSet = new ProjectSettings("staff", PAGE_LIST);
		if($lSet->GetTableData(".sqlquery"))
			$fieldList = $lSet->GetTableData(".sqlquery")->toSql();
		if($fieldList)
		{
			if(!$this->pSet->isCaseInsensitiveUsername()) {
				$where = GetFullFieldName($cUserNameField,"staff",false).
				   "=".$strUsername." and ".GetFullFieldName($cPasswordField,"staff",false)."=".$strPassword;
   			} else {
				$where = db_upper(getFullFieldName($cUserNameField,"staff",false)).
					   "=".$this->pSet->getCaseSensitiveUsername($strUsername)." and ".GetFullFieldName($cPasswordField,"staff",false).
					   "=".$strPassword;
			}		   
