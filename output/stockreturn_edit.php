...
$layout->blocks["top"][] = "details";$page_layouts["stockreturn_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "stockreturn_inline_edit.htm" : "stockreturn_edit.htm";
...
//	processing ProductID - begin
	...
	if($condition)
	{
		...
		$control_ProductID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing TimeStamp - begin
	$condition = 1;

	if($condition)
	{
		$control_TimeStamp = $pageObject->getControl("TimeStamp", $id);
		$control_TimeStamp->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing TimeStamp - end
//	processing ReceiveFlag - begin
	$condition = 1;

	if($condition)
	{
		$control_ReceiveFlag = $pageObject->getControl("ReceiveFlag", $id);
		$control_ReceiveFlag->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: stockreturn_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: stockreturn_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["TimeStamp"] = $evalues["TimeStamp"];
	$data["ReceiveFlag"] = $evalues["ReceiveFlag"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("customerID_label","<label for=\"".GetInputElementId("customerID", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ProductID_label","<label for=\"".GetInputElementId("ProductID", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("TimeStamp_label","<label for=\"".GetInputElementId("TimeStamp", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ReceiveFlag_label","<label for=\"".GetInputElementId("ReceiveFlag", $id, PAGE_EDIT)."\">","</label>");
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "stockreturn";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "stockreturn";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='stockreturn_view.php?".$viewlink."'\"");
