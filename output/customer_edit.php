...
$layout->blocks["top"][] = "details";$page_layouts["customer_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "customer_inline_edit.htm" : "customer_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["CID"] = urldecode(postvalue("editid1"));
$savedKeys["CID"] = urldecode(postvalue("editid1"));
...
//	processing BRegNo - begin
	$condition = 1;

	if($condition)
	{
		$control_BRegNo = $pageObject->getControl("BRegNo", $id);
		$control_BRegNo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing BRegNo - end
//	processing ComName - begin
	$condition = 1;

	if($condition)
	{
		$control_ComName = $pageObject->getControl("ComName", $id);
		$control_ComName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Fname - begin
	...
	if($condition)
	{
		...
		$control_Fname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Lname - begin
	...
	if($condition)
	{
		...
		$control_Lname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Lname - end
//	processing OAddress - begin
	$condition = 1;

	if($condition)
	{
		$control_OAddress = $pageObject->getControl("OAddress", $id);
		$control_OAddress->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing OAddress - end
//	processing DAddress - begin
	$condition = 1;

	if($condition)
	{
		$control_DAddress = $pageObject->getControl("DAddress", $id);
		$control_DAddress->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing DAddress - end
//	processing Phone - begin
	...
	if($condition)
	{
		...
		$control_Phone->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing StaffID - end
	...
	//	do event
		...
		{		
			...
			//	processing BRegNo - begin
				...
				if($condition)
				{
					$control_BRegNo->afterSuccessfulSave();
				}
	//	processing BRegNo - end
			//	processing ComName - begin
							$condition = 1;
	...
	//	processing Lname - end
			//	processing OAddress - begin
				...
				if($condition)
				{
					$control_OAddress->afterSuccessfulSave();
				}
	//	processing OAddress - end
			//	processing DAddress - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_DAddress->afterSuccessfulSave();
				}
	//	processing DAddress - end
			//	processing Phone - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["CID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: customer_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: customer_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["BRegNo"] = $evalues["BRegNo"];
	$data["ComName"] = $evalues["ComName"];
	$data["Fname"] = $evalues["Fname"];
	$data["Lname"] = $evalues["Lname"];
	$data["OAddress"] = $evalues["OAddress"];
	$data["DAddress"] = $evalues["DAddress"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("BRegNo"))
		$xt->assign("BRegNo_fieldblock",true);
	else
		$xt->assign("BRegNo_tabfieldblock",true);
	$xt->assign("BRegNo_label",true);
	if(isEnableSection508())
		$xt->assign_section("BRegNo_label","<label for=\"".GetInputElementId("BRegNo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ComName_label","<label for=\"".GetInputElementId("ComName", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Fname_label","<label for=\"".GetInputElementId("Fname", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Lname_label","<label for=\"".GetInputElementId("Lname", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("OAddress"))
		$xt->assign("OAddress_fieldblock",true);
	else
		$xt->assign("OAddress_tabfieldblock",true);
	$xt->assign("OAddress_label",true);
	if(isEnableSection508())
		$xt->assign_section("OAddress_label","<label for=\"".GetInputElementId("OAddress", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("DAddress"))
		$xt->assign("DAddress_fieldblock",true);
	else
		$xt->assign("DAddress_tabfieldblock",true);
	$xt->assign("DAddress_label",true);
	if(isEnableSection508())
		$xt->assign_section("DAddress_label","<label for=\"".GetInputElementId("DAddress", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Phone_label","<label for=\"".GetInputElementId("Phone", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("CID", $data)));
...
//	BRegNo - 
	$value = $pageObject->showDBValue("BRegNo", $data, $keylink);
	$showValues["BRegNo"] = $value;
	$showFields[] = "BRegNo";
		$showRawValues["BRegNo"] = substr($data["BRegNo"],0,100);
...
//	Fname - 
	...
	$showValues["Fname"] = $value;
	$showFields[] = "Fname";
		$showRawValues["Fname"] = substr($data["Fname"],0,100);

//	Lname - 
	$value = $pageObject->showDBValue("Lname", $data, $keylink);
	$showValues["Lname"] = $value;
	$showFields[] = "Lname";
		$showRawValues["Lname"] = substr($data["Lname"],0,100);

//	OAddress - 
	$value = $pageObject->showDBValue("OAddress", $data, $keylink);
	$showValues["OAddress"] = $value;
	$showFields[] = "OAddress";
		$showRawValues["OAddress"] = substr($data["OAddress"],0,100);

//	DAddress - 
	$value = $pageObject->showDBValue("DAddress", $data, $keylink);
	$showValues["DAddress"] = $value;
	$showFields[] = "DAddress";
		$showRawValues["DAddress"] = substr($data["DAddress"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='customer_view.php?".$viewlink."'\"");
