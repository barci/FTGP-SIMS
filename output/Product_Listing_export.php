...
$layout->blocks["top"][] = "export";$page_layouts["Product_Listing_export"] = $layout;
...
$xt->display("Product_Listing_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Product_Listing.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Product_Listing.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Product_Listing.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["longname"] = $pageObject->showDBValue("longname", $row);
			$values["Pname"] = $pageObject->showDBValue("Pname", $row);
			$values["CatName"] = $pageObject->showDBValue("CatName", $row);
			$values["Usize"] = $pageObject->showDBValue("Usize", $row);
			$values["Desc"] = $pageObject->showDBValue("Desc", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Product_Listing.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["longname"] = $pageObject->getViewControl("longname")->showDBValue($row, "");
			$values["Pname"] = $pageObject->getViewControl("Pname")->showDBValue($row, "");
			$values["CatName"] = $pageObject->getViewControl("CatName")->showDBValue($row, "");
			$values["Usize"] = $pageObject->getViewControl("Usize")->showDBValue($row, "");
			$values["Desc"] = $pageObject->getViewControl("Desc")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["longname"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Pname"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["CatName"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Usize"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Desc"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Product ID").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Product Details").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Product Name").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Category Name").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["longname"]);
					else
						echo $values["longname"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["CatName"]);
					else
						echo $values["CatName"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Desc"]);
					else
						echo $values["Desc"];
			echo '</td>';
			echo "</tr>";
