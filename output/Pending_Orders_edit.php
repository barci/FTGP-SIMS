...
include("include/Pending_Orders_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Pending_Orders_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Pending_Orders_inline_edit.htm" : "Pending_Orders_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["ODetailID"] = urldecode(postvalue("editid1"));
$savedKeys["ODetailID"] = urldecode(postvalue("editid1"));
...
//	processing OrdQuant - begin
	$condition = 1;

	if($condition)
	{
		$control_OrdQuant = $pageObject->getControl("OrdQuant", $id);
		$control_OrdQuant->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["ODetailID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: Pending_Orders_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Pending_Orders_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("OrdQuant"))
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("ODetailID", $data)));
...
//	ODetailID - 
	$value = $pageObject->showDBValue("ODetailID", $data, $keylink);
	$showValues["ODetailID"] = $value;
	$showFields[] = "ODetailID";
...
//	BillNo - 
	$value = $pageObject->showDBValue("BillNo", $data, $keylink);
	$showValues["BillNo"] = $value;
	$showFields[] = "BillNo";
...
//	DelQuant - 
	$value = $pageObject->showDBValue("DelQuant", $data, $keylink);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Pending Orders";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Pending_Orders_view.php?".$viewlink."'\"");
