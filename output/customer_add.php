...
$layout->blocks["top"][] = "details";$page_layouts["customer_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "customer_inline_add.htm";
else
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing BRegNo - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_BRegNo = $pageObject->getControl("BRegNo", $id);
		$control_BRegNo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing OAddress - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_OAddress = $pageObject->getControl("OAddress", $id);
		$control_OAddress->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing OAddress - end
//	processing DAddress - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DAddress = $pageObject->getControl("DAddress", $id);
		$control_DAddress->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'customer_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'customer_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: customer_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["BRegNo"]=@$avalues["BRegNo"];
	$defvalues["ComName"]=@$avalues["ComName"];
	$defvalues["Fname"]=@$avalues["Fname"];
	$defvalues["Lname"]=@$avalues["Lname"];
	$defvalues["OAddress"]=@$avalues["OAddress"];
	$defvalues["DAddress"]=@$avalues["DAddress"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("BRegNo_label","<label for=\"".GetInputElementId("BRegNo", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Fname_label","<label for=\"".GetInputElementId("Fname", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Lname_label","<label for=\"".GetInputElementId("Lname", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("OAddress_label","<label for=\"".GetInputElementId("OAddress", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("DAddress_label","<label for=\"".GetInputElementId("DAddress", $id, PAGE_ADD)."\">","</label>");
