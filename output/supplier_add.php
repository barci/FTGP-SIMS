...
$layout->blocks["top"][] = "details";$page_layouts["supplier_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "supplier_inline_add.htm";
else
	$templatefile = "supplier_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing ComName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ComName = $pageObject->getControl("ComName", $id);
		$control_ComName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ComName - end
//	processing ConFName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ConFName = $pageObject->getControl("ConFName", $id);
		$control_ConFName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ConFName - end
//	processing ConLName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ConLName = $pageObject->getControl("ConLName", $id);
		$control_ConLName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ConLName - end
//	processing ConTitle - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ConTitle = $pageObject->getControl("ConTitle", $id);
		$control_ConTitle->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ConTitle - end
...
//	processing Fax - end
//	processing PayMeth - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_PayMeth = $pageObject->getControl("PayMeth", $id);
		$control_PayMeth->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing PayMeth - end
//	processing DisType - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DisType = $pageObject->getControl("DisType", $id);
		$control_DisType->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DisType - end
...
//	processing ComName - start
			...
			if($inlineAddOption)
			{
				$control_ComName->afterSuccessfulSave();
			}
//	processing ComName - end
//	processing ConFName - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_ConFName->afterSuccessfulSave();
			}
//	processing ConFName - end
//	processing ConLName - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_ConLName->afterSuccessfulSave();
			}
//	processing ConLName - end
//	processing ConTitle - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_ConTitle->afterSuccessfulSave();
			}
//	processing ConTitle - end
//	processing Address - start
			$inlineAddOption = true;
...
//	processing PayMeth - start
			...
			if($inlineAddOption)
			{
				$control_PayMeth->afterSuccessfulSave();
			}
//	processing PayMeth - end
//	processing DisType - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_DisType->afterSuccessfulSave();
			}
//	processing DisType - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'supplier_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'supplier_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: supplier_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["SupplierID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["SupplierID"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["ComName"]=@$avalues["ComName"];
	$defvalues["ConFName"]=@$avalues["ConFName"];
	$defvalues["ConLName"]=@$avalues["ConLName"];
	$defvalues["ConTitle"]=@$avalues["ConTitle"];
	$defvalues["Address"]=@$avalues["Address"];
	$defvalues["Phone"]=@$avalues["Phone"];
	$defvalues["Fax"]=@$avalues["Fax"];
	$defvalues["Email"]=@$avalues["Email"];
	$defvalues["PayMeth"]=@$avalues["PayMeth"];
	$defvalues["DisType"]=@$avalues["DisType"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("ComName_label","<label for=\"".GetInputElementId("ComName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ConFName_label","<label for=\"".GetInputElementId("ConFName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ConLName_label","<label for=\"".GetInputElementId("ConLName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ConTitle_label","<label for=\"".GetInputElementId("ConTitle", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Phone_label","<label for=\"".GetInputElementId("Phone", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("PayMeth_label","<label for=\"".GetInputElementId("PayMeth", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("DisType_label","<label for=\"".GetInputElementId("DisType", $id, PAGE_ADD)."\">","</label>");
...
//	DisType
	...
	{	
		...
		$showValues["DisType"] = $value;
		$showFields[] = "DisType";
	}	
		$showRawValues["SupplierID"] = substr($data["SupplierID"],0,100);
		$showRawValues["ComName"] = substr($data["ComName"],0,100);
		$showRawValues["ConFName"] = substr($data["ConFName"],0,100);
		$showRawValues["ConLName"] = substr($data["ConLName"],0,100);
		$showRawValues["ConTitle"] = substr($data["ConTitle"],0,100);
		...
		$showRawValues["PayMeth"] = substr($data["PayMeth"],0,100);
		$showRawValues["DisType"] = substr($data["DisType"],0,100);
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="product" && postvalue("field")=="SuppID") 
	{
		$lookupFieldName = "SuppID";
		$lookupTable = "supplier";
		$linkFieldName = "SupplierID";
		$dispfield = "ComName";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("product", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "product", "SuppID");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("SupplierID", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",ComName";
		}
	}
	if(postvalue("table")=="Product_Order_List" && postvalue("field")=="SuppID") 
	{
		$lookupFieldName = "SuppID";
		$lookupTable = "supplier";
		$linkFieldName = "SupplierID";
		$dispfield = "ComName";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("Product-Order List", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Product-Order List", "SuppID");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
