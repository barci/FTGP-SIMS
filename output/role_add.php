...
$layout->blocks["top"][] = "details";$page_layouts["role_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "role_inline_add.htm";
else
	$templatefile = "role_add.htm";
...
//	processing RoleID - start
	...
	if($inlineAddOption)
	{
		...
		$control_RoleID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing RoleID - end
//	processing RoleName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_RoleName = $pageObject->getControl("RoleName", $id);
		$control_RoleName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'role_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'role_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: role_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["RoleID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["RoleID"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["RoleID"]=@$avalues["RoleID"];
	$defvalues["RoleName"]=@$avalues["RoleName"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("RoleID_label","<label for=\"".GetInputElementId("RoleID", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("RoleName_label","<label for=\"".GetInputElementId("RoleName", $id, PAGE_ADD)."\">","</label>");
