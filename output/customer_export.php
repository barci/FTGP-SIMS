...
$layout->blocks["top"][] = "export";$page_layouts["customer_export"] = $layout;
...
$xt->display("customer_export.htm");
...
function ExportToXML($cipherer)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["BRegNo"] = $pageObject->showDBValue("BRegNo", $row);
			...
			$values["OAddress"] = $pageObject->showDBValue("OAddress", $row);
			$values["DAddress"] = $pageObject->showDBValue("DAddress", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["BRegNo"] = $pageObject->getViewControl("BRegNo")->showDBValue($row, "");
			...
			$values["OAddress"] = $pageObject->getViewControl("OAddress")->showDBValue($row, "");
			$values["DAddress"] = $pageObject->getViewControl("DAddress")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["BRegNo"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["OAddress"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DAddress"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("BReg No").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Office Address").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Delivery Address").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["BRegNo"]);
					else
						echo $values["BRegNo"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["OAddress"]);
					else
						echo $values["OAddress"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DAddress"]);
					else
						echo $values["DAddress"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
