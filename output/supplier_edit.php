...
$layout->blocks["top"][] = "details";$page_layouts["supplier_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "supplier_inline_edit.htm" : "supplier_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["SupplierID"] = urldecode(postvalue("editid1"));
$savedKeys["SupplierID"] = urldecode(postvalue("editid1"));
...
//	processing ConFName - begin
	$condition = 1;

	if($condition)
	{
		$control_ConFName = $pageObject->getControl("ConFName", $id);
		$control_ConFName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ConFName - end
//	processing ConLName - begin
	$condition = 1;

	if($condition)
	{
		$control_ConLName = $pageObject->getControl("ConLName", $id);
		$control_ConLName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ConLName - end
//	processing ConTitle - begin
	$condition = 1;

	if($condition)
	{
		$control_ConTitle = $pageObject->getControl("ConTitle", $id);
		$control_ConTitle->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing PayMeth - begin
	$condition = 1;

	if($condition)
	{
		$control_PayMeth = $pageObject->getControl("PayMeth", $id);
		$control_PayMeth->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing PayMeth - end
//	processing DisType - begin
	$condition = 1;

	if($condition)
	{
		$control_DisType = $pageObject->getControl("DisType", $id);
		$control_DisType->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["SupplierID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: supplier_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["ConFName"] = $evalues["ConFName"];
	$data["ConLName"] = $evalues["ConLName"];
	$data["ConTitle"] = $evalues["ConTitle"];
	$data["Address"] = $evalues["Address"];
	$data["Phone"] = $evalues["Phone"];
	$data["Fax"] = $evalues["Fax"];
	$data["Email"] = $evalues["Email"];
	$data["PayMeth"] = $evalues["PayMeth"];
	$data["DisType"] = $evalues["DisType"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("ConFName_label","<label for=\"".GetInputElementId("ConFName", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ConLName_label","<label for=\"".GetInputElementId("ConLName", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ConTitle_label","<label for=\"".GetInputElementId("ConTitle", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("PayMeth_label","<label for=\"".GetInputElementId("PayMeth", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("DisType_label","<label for=\"".GetInputElementId("DisType", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='supplier_view.php?".$viewlink."'\"");
