...
add_nocache_headers();
include("include/Product_List_View_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Product_List_View_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Product_List_View_inline_add.htm";
else
	$templatefile = "Product_List_View_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Product_List_View_add.php');
...
//	processing Product Name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Product_Name = $pageObject->getControl("Product Name", $id);
		$control_Product_Name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Product Name - end
...
//	processing Product Name - start
			...
			if($inlineAddOption)
			{
				$control_Product_Name->afterSuccessfulSave();
			}
//	processing Product Name - end
//	processing CatName - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Product_List_View_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Product_List_View_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Product_List_View_".$pageObject->getPageType().".php");
...
//	Product Name
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Product Name", $data, $keylink);
		$showValues["Product Name"] = $value;
		$showFields[] = "Product Name";
	}	
//	CatName
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
...
//	Desc
	...
	{	
		...
		$showRawValues["Product Name"] = substr($data["Product Name"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Product List View";
		$options['firstTime'] = 1;
