...
function checkTableName($shortTName, $type=false)
{
	...
	if ("supplier" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("category" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("product" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("customer" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("role" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("staff" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Product_Order_List" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
...
function inFacebook(){
	...
	if ($signed_request) {
		...
		$strSQL = "select * from ".AddTableWrappers("staff")." where ".AddFieldWrappers(GetUserNameField())."='".$strUsername."'";
...
function AfterFBLogIn($pUsername, $pPassword){
	...
	$strSQL = "select * from ".AddTableWrappers("staff")." where ".AddFieldWrappers($cUserNameField)."=".$strUsername."";
	$rs = db_query($strSQL,$conn);
 	$data = db_fetch_array($rs);
	if($data){
		$logged=true;
