...
include("include/Product_Listing_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Product_Listing_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Product_Listing_inline_edit.htm" : "Product_Listing_edit.htm";
...
//	processing prodid - begin
	...
	if($condition)
	{
		...
		$control_prodid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing prodid - end
//	processing longname - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_longname = $pageObject->getControl("longname", $id);
		$control_longname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing longname - end
//	processing Pname - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Pname = $pageObject->getControl("Pname", $id);
		$control_Pname->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing Pname - end
//	processing CatName - begin
	$condition = 1;

	if($condition)
	{
		$control_CatName = $pageObject->getControl("CatName", $id);
		$control_CatName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing CatName - end
//	processing Usize - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Usize = $pageObject->getControl("Usize", $id);
		$control_Usize->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing Usize - end
//	processing Desc - begin
	$condition = 1;

	if($condition)
	{
		$control_Desc = $pageObject->getControl("Desc", $id);
		$control_Desc->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing Desc - end
	...
	//	processing prodid - end
			//	processing longname - begin
				...
				if($condition)
				{
					$control_longname->afterSuccessfulSave();
				}
	//	processing longname - end
			//	processing Pname - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing Pname - end
			//	processing CatName - begin
				...
				if($condition)
				{
					$control_CatName->afterSuccessfulSave();
				}
	//	processing CatName - end
			//	processing Usize - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing Usize - end
			//	processing Desc - begin
				...
				if($condition)
				{
					$control_Desc->afterSuccessfulSave();
				}
	//	processing Desc - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Product_Listing_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Product_Listing_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["longname"] = $evalues["longname"];
	$data["Pname"] = $evalues["Pname"];
	$data["CatName"] = $evalues["CatName"];
	$data["Usize"] = $evalues["Usize"];
	$data["Desc"] = $evalues["Desc"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("CatName"))
		$xt->assign("CatName_fieldblock",true);
	else
		$xt->assign("CatName_tabfieldblock",true);
	$xt->assign("CatName_label",true);
	if(isEnableSection508())
		$xt->assign_section("CatName_label","<label for=\"".GetInputElementId("CatName", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Desc"))
		$xt->assign("Desc_fieldblock",true);
	else
		$xt->assign("Desc_tabfieldblock",true);
	$xt->assign("Desc_label",true);
	if(isEnableSection508())
		$xt->assign_section("Desc_label","<label for=\"".GetInputElementId("Desc", $id, PAGE_EDIT)."\">","</label>");
...
//	longname - 
	...
	$showValues["longname"] = $value;
	$showFields[] = "longname";
		$showRawValues["longname"] = substr($data["longname"],0,100);
...
//	CatName - 
	$value = $pageObject->showDBValue("CatName", $data, $keylink);
	$showValues["CatName"] = $value;
	$showFields[] = "CatName";
		$showRawValues["CatName"] = substr($data["CatName"],0,100);
...
//	Desc - 
	$value = $pageObject->showDBValue("Desc", $data, $keylink);
	$showValues["Desc"] = $value;
	$showFields[] = "Desc";
		$showRawValues["Desc"] = substr($data["Desc"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Product Listing";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Product_Listing_view.php?".$viewlink."'\"");
