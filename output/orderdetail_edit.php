...
include("include/orderdetail_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["orderdetail_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "orderdetail_inline_edit.htm" : "orderdetail_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: orderdetail_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: orderdetail_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "orderdetail";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='orderdetail_view.php?".$viewlink."'\"");
