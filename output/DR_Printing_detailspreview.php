...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["DR_Printing_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("DrNo", $strTableName, false)."=".make_db_value("DrNo",$_SESSION[$strTableName."_masterkey1"]);
		$where.=" and ";
	$where .= GetFullFieldName("orderID", $strTableName, false)."=".make_db_value("orderID",$_SESSION[$strTableName."_masterkey2"]);
		$where.=" and ";
	$where .= GetFullFieldName("eta", $strTableName, false)."=".make_db_value("eta",$_SESSION[$strTableName."_masterkey3"]);
}
...
if($rowcount) {
	...
	//	USP - Custom
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("USP", $data, $keylink);
			$row["USP_value"] = $value;
	//	subtotal - 
			$viewContainer->recId = $recordsCounter;
	...
	//	Destination - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Destination", $data, $keylink);
			$row["Destination_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("DR_Printing_detailspreview.htm");
