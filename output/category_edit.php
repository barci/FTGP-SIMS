...
$layout->blocks["top"][] = "details";$page_layouts["category_edit"] = $layout;
...
$layout->blocks["top"][] = "recordcontrols";$page_layouts["product_list"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "category_inline_edit.htm" : "category_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["CatID"] = urldecode(postvalue("editid1"));
$savedKeys["CatID"] = urldecode(postvalue("editid1"));
...
if($inlineedit!=EDIT_INLINE)
{
	...
	{
		$ids = $id;
			$dpPermis = $pageObject->getPermissions("product");
		if($dpPermis['search'] || $dpPermis['edit']){
			$mKeys["product"] = $pageObject->pSet->getMasterKeysByDetailTable("product");
			$dpParams['strTableNames'][] = "product";
			$dpParams['ids'][] = ++$ids;
		}
...
//	processing CatID - begin
	$condition = 1;

	if($condition)
	{
		$control_CatID = $pageObject->getControl("CatID", $id);
		$control_CatID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_CatID->getWebValue()!==false)
			$keys["CatID"] = $control_CatID->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["CatID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: category_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: category_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("CatID_label","<label for=\"".GetInputElementId("CatID", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("CatID", $data)));
...
//	detail tables
	$showDetailKeys["product"]["masterkey1"] = $data["CatID"];		
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='category_view.php?".$viewlink."'\"");
