...
include("include/role_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["role_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "role_inline_edit.htm" : "role_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["RoleID"] = urldecode(postvalue("editid1"));
$savedKeys["RoleID"] = urldecode(postvalue("editid1"));
...
//	processing RoleID - begin
	...
	if($condition)
	{
		...
		$control_RoleID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_RoleID->getWebValue()!==false)
			$keys["RoleID"] = $control_RoleID->getWebValue();
	}
//	processing RoleID - end
//	processing RoleName - begin
	$condition = 1;

	if($condition)
	{
		$control_RoleName = $pageObject->getControl("RoleName", $id);
		$control_RoleName->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing RoleName - end
...
//	processing Desc - end
	...
	//	processing RoleID - end
			//	processing RoleName - begin
				...
				if($condition)
				{
					$control_RoleName->afterSuccessfulSave();
				}
	//	processing RoleName - end
			//	processing Desc - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["RoleID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: role_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: role_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["RoleID"] = $evalues["RoleID"];
	$data["RoleName"] = $evalues["RoleName"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("RoleID"))
		$xt->assign("RoleID_fieldblock",true);
	else
		$xt->assign("RoleID_tabfieldblock",true);
	$xt->assign("RoleID_label",true);
	if(isEnableSection508())
		$xt->assign_section("RoleID_label","<label for=\"".GetInputElementId("RoleID", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("RoleName"))
		$xt->assign("RoleName_fieldblock",true);
	else
		$xt->assign("RoleName_tabfieldblock",true);
	$xt->assign("RoleName_label",true);
	if(isEnableSection508())
		$xt->assign_section("RoleName_label","<label for=\"".GetInputElementId("RoleName", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("RoleID", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["RoleID"]));


//	RoleID - 
	$value = $pageObject->showDBValue("RoleID", $data, $keylink);
	$showValues["RoleID"] = $value;
	$showFields[] = "RoleID";
		$showRawValues["RoleID"] = substr($data["RoleID"],0,100);

//	RoleName - 
	$value = $pageObject->showDBValue("RoleName", $data, $keylink);
	$showValues["RoleName"] = $value;
	$showFields[] = "RoleName";
		$showRawValues["RoleName"] = substr($data["RoleName"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='role_view.php?".$viewlink."'\"");
