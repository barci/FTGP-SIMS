...
$layout->blocks["top"][] = "export";$page_layouts["stockreturn_export"] = $layout;
...
$xt->display("stockreturn_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=stockreturn.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=stockreturn.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=stockreturn.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["customerID"] = $pageObject->showDBValue("customerID", $row);
			...
			$values["ReceiveFlag"] = $pageObject->showDBValue("ReceiveFlag", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=stockreturn.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["customerID"] = $pageObject->getViewControl("customerID")->showDBValue($row, "");
			...
			$values["ReceiveFlag"] = $pageObject->getViewControl("ReceiveFlag")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ReceiveFlag"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Receive Flag").'</td>';	
	}
	else
	{
		echo "<td>"."ID"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["customerID"]);
					else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["ProductID"]);
					else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["ReceiveFlag"]);
					else
						echo $values["ReceiveFlag"];
			echo '</td>';
			echo "</tr>";
		}
