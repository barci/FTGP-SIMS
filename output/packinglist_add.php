...
$layout->blocks["top"][] = "details";$page_layouts["packinglist_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "packinglist_inline_add.htm";
else
	$templatefile = "packinglist_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: packinglist_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing delNo - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_delNo = $pageObject->getControl("delNo", $id);
		$control_delNo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing delNo - end
//	processing orderNo - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_orderNo = $pageObject->getControl("orderNo", $id);
		$control_orderNo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing orderNo - end
//	processing delID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_delID = $pageObject->getControl("delID", $id);
		$control_delID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing delID - end
...
//	processing qty - end
//	processing unit - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_unit = $pageObject->getControl("unit", $id);
		$control_unit->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing unit - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="DR Printing")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["delID"]==""){
			$avalues["delID"] = prepare_for_db("delID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
		...
		if($avalues["delNo"]==""){
			$avalues["delNo"] = prepare_for_db("delNo",$_SESSION[$sessionPrefix."_masterkey2"]);
		}
		...
		if($avalues["orderNo"]==""){
			$avalues["orderNo"] = prepare_for_db("orderNo",$_SESSION[$sessionPrefix."_masterkey3"]);
		}
		...
		if($avalues["date"]==""){
			$avalues["date"] = prepare_for_db("date",$_SESSION[$sessionPrefix."_masterkey4"]);
		}
...
//	processing delNo - start
			...
			if($inlineAddOption)
			{
				$control_delNo->afterSuccessfulSave();
			}
//	processing delNo - end
//	processing orderNo - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_orderNo->afterSuccessfulSave();
			}
//	processing orderNo - end
//	processing delID - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_delID->afterSuccessfulSave();
			}
//	processing delID - end
//	processing qty - start
			$inlineAddOption = true;
...
//	processing unit - start
			...
			if($inlineAddOption)
			{
				$control_unit->afterSuccessfulSave();
			}
//	processing unit - end
//	processing date - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'packinglist_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'packinglist_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: packinglist_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="DR Printing")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["delID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
	if(postvalue("masterkey2"))
		$_SESSION[$sessionPrefix."_masterkey2"] = postvalue("masterkey2");

	if(postvalue("mainMPageType")<>"add")
		$defvalues["delNo"] = @$_SESSION[$sessionPrefix."_masterkey2"];	
	
	if(postvalue("masterkey3"))
		$_SESSION[$sessionPrefix."_masterkey3"] = postvalue("masterkey3");

	if(postvalue("mainMPageType")<>"add")
		$defvalues["orderNo"] = @$_SESSION[$sessionPrefix."_masterkey3"];	
	
	if(postvalue("masterkey4"))
		$_SESSION[$sessionPrefix."_masterkey4"] = postvalue("masterkey4");

	if(postvalue("mainMPageType")<>"add")
		$defvalues["date"] = @$_SESSION[$sessionPrefix."_masterkey4"];	
	
}

if($readavalues)
{
	$defvalues["delNo"]=@$avalues["delNo"];
	$defvalues["orderNo"]=@$avalues["orderNo"];
	$defvalues["delID"]=@$avalues["delID"];
	$defvalues["notes"]=@$avalues["notes"];
	$defvalues["qty"]=@$avalues["qty"];
	$defvalues["unit"]=@$avalues["unit"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("delNo"))
		$xt->assign("delNo_fieldblock",true);
	else
		$xt->assign("delNo_tabfieldblock",true);
	$xt->assign("delNo_label",true);
	if(isEnableSection508())
		$xt->assign_section("delNo_label","<label for=\"".GetInputElementId("delNo", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("orderNo"))
		$xt->assign("orderNo_fieldblock",true);
	else
		$xt->assign("orderNo_tabfieldblock",true);
	$xt->assign("orderNo_label",true);
	if(isEnableSection508())
		$xt->assign_section("orderNo_label","<label for=\"".GetInputElementId("orderNo", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("delID"))
		$xt->assign("delID_fieldblock",true);
	else
		$xt->assign("delID_tabfieldblock",true);
	$xt->assign("delID_label",true);
	if(isEnableSection508())
		$xt->assign_section("delID_label","<label for=\"".GetInputElementId("delID", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("unit"))
		$xt->assign("unit_fieldblock",true);
	else
		$xt->assign("unit_tabfieldblock",true);
	$xt->assign("unit_label",true);
	if(isEnableSection508())
		$xt->assign_section("unit_label","<label for=\"".GetInputElementId("unit", $id, PAGE_ADD)."\">","</label>");
...
//	delNo
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("delNo", $data, $keylink);
		$showValues["delNo"] = $value;
		$showFields[] = "delNo";
	}	
//	orderNo
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("orderNo", $data, $keylink);
		$showValues["orderNo"] = $value;
		$showFields[] = "orderNo";
	}	
//	delID
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("delID", $data, $keylink);
		$showValues["delID"] = $value;
		$showFields[] = "delID";
	}	
//	notes
	$display = false;
...
//	unit
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("unit", $data, $keylink);
		$showValues["unit"] = $value;
		$showFields[] = "unit";
	}	
//	date
	$display = false;
...
//	staffID
	...
	{	
		...
		$showRawValues["delNo"] = substr($data["delNo"],0,100);
		$showRawValues["orderNo"] = substr($data["orderNo"],0,100);
		$showRawValues["delID"] = substr($data["delID"],0,100);
		$showRawValues["notes"] = substr($data["notes"],0,100);
		$showRawValues["qty"] = substr($data["qty"],0,100);
		$showRawValues["unit"] = substr($data["unit"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "packinglist";
		$options['firstTime'] = 1;
