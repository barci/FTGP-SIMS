...
$layout->blocks["top"][] = "details";$page_layouts["Delivery_List_View_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Delivery_List_View_inline_add.htm";
else
	$templatefile = "Delivery_List_View_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Delivery_List_View_add.php');
...
//	processing longname - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_longname = $pageObject->getControl("longname", $id);
		$control_longname->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing TimeStamp - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_TimeStamp = $pageObject->getControl("TimeStamp", $id);
		$control_TimeStamp->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing TimeStamp - end
...
//	processing TimeStamp - start
			...
			if($inlineAddOption)
			{
				$control_TimeStamp->afterSuccessfulSave();
			}
//	processing TimeStamp - end
//	processing staffID - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Delivery_List_View_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Delivery_List_View_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Delivery_List_View_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["TimeStamp"]=@$avalues["TimeStamp"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("TimeStamp_label","<label for=\"".GetInputElementId("TimeStamp", $id, PAGE_ADD)."\">","</label>");
