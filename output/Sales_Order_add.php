...
$layout->blocks["top"][] = "details";$page_layouts["Sales_Order_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Sales_Order_inline_add.htm";
else
	$templatefile = "Sales_Order_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Sales_Order_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Sales_Order_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Sales_Order_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Sales_Order_".$pageObject->getPageType().".php");
