...
$layout->blocks["top"][] = "details";$page_layouts["category_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "category_inline_add.htm";
else
...
//	processing CatName - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CatName = $pageObject->getControl("CatName", $id);
		$control_CatName->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CatName - end
//	processing Desc - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Desc = $pageObject->getControl("Desc", $id);
		$control_Desc->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Desc - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'category_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'category_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: category_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["CatID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["CatID"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["CatName"]=@$avalues["CatName"];
	$defvalues["Desc"]=@$avalues["Desc"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("CatName_label","<label for=\"".GetInputElementId("CatName", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Desc_label","<label for=\"".GetInputElementId("Desc", $id, PAGE_ADD)."\">","</label>");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="product" && postvalue("field")=="CatID") 
	{
		$lookupFieldName = "CatID";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "product", "CatID");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("CatID", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",CatName";
		}
	}
	if(postvalue("table")=="Product_Order_List" && postvalue("field")=="CatID") 
	{
		$lookupFieldName = "CatID";
		$lookupTable = "category";
		$linkFieldName = "CatID";
		...
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "Product-Order List", "CatID");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
