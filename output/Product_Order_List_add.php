...
$layout->blocks["top"][] = "details";$page_layouts["Product_Order_List_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Product_Order_List_inline_add.htm";
else
	$templatefile = "Product_Order_List_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Product_Order_List_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Product_Order_List_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Product_Order_List_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Product_Order_List_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Product-Order List";
		$options['firstTime'] = 1;
