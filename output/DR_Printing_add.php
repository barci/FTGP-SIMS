...
$layout->blocks["top"][] = "details";$page_layouts["DR_Printing_add"] = $layout;
...
$layout->blocks["top"][] = "recordcontrols";$page_layouts["packinglist_list"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "DR_Printing_inline_add.htm";
else
	$templatefile = "DR_Printing_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: DR_Printing_add.php');
...
if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER || $inlineadd==ADD_POPUP)
{
	...
	if($pageObject->isShowDetailTables  && !isMobile())
	{
		...
		$countDetailsIsShow = 0;
			$dpPermis = $pageObject->getPermissions("packinglist");
		if ($dpPermis['add']){
			$countDetailsIsShow ++;
			$mKeys["packinglist"] = $pageObject->pSet->getMasterKeysByDetailTable("packinglist");
			$dpParams['strTableNames'][] = "packinglist";
			$dpParams['ids'][] = ++$ids;
		}
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing DrNo - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_DrNo = $pageObject->getControl("DrNo", $id);
		$control_DrNo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing DrNo - end
//	processing orderID - start
	...
	if($inlineAddOption)
	{
		...
		$control_orderID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing orderID - end
//	processing prodID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_prodID = $pageObject->getControl("prodID", $id);
		$control_prodID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing USP - end
//	processing subtotal - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_subtotal = $pageObject->getControl("subtotal", $id);
		$control_subtotal->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing subtotal - end
//	processing eta - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
...
//	processing delFlag - start
	...
	if($inlineAddOption)
	{
		$control_delFlag = $pageObject->getControl("delFlag", $id);
		$control_delFlag->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing delFlag - end
//	processing Destination - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Destination = $pageObject->getControl("Destination", $id);
		$control_Destination->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Destination - end
...
//	processing prodID - start
			...
			if($inlineAddOption)
			{
				$control_prodID->afterSuccessfulSave();
			}
//	processing prodID - end
//	processing qty - start
			$inlineAddOption = true;
...
//	processing subtotal - start
			...
			if($inlineAddOption)
			{
				$control_subtotal->afterSuccessfulSave();
			}
//	processing subtotal - end
//	processing eta - start
			$inlineAddOption = true;
...
//	processing delFlag - start
			...
			if($inlineAddOption)
			{
				$control_delFlag->afterSuccessfulSave();
			}
//	processing delFlag - end
//	processing Destination - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_Destination->afterSuccessfulSave();
			}
//	processing Destination - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'DR_Printing_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'DR_Printing_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: DR_Printing_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["DrNo"]=@$avalues["DrNo"];
	$defvalues["orderID"]=@$avalues["orderID"];
	$defvalues["prodID"]=@$avalues["prodID"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("DrNo_label","<label for=\"".GetInputElementId("DrNo", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("orderID_label","<label for=\"".GetInputElementId("orderID", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("prodID_label","<label for=\"".GetInputElementId("prodID", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("eta_label","<label for=\"".GetInputElementId("eta", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["packinglist"]["masterkey1"] = $data["ID"];	
	$showDetailKeys["packinglist"]["masterkey2"] = $data["DrNo"];	
	$showDetailKeys["packinglist"]["masterkey3"] = $data["orderID"];	
	$showDetailKeys["packinglist"]["masterkey4"] = $data["eta"];	
...
//	subtotal
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("subtotal", $data, $keylink);
		$showValues["subtotal"] = $value;
		$showFields[] = "subtotal";
	}	
//	eta
	$display = false;
...
//	Destination
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Destination", $data, $keylink);
		$showValues["Destination"] = $value;
		...
		$showRawValues["subtotal"] = substr($data["subtotal"],0,100);
		$showRawValues["eta"] = substr($data["eta"],0,100);
		$showRawValues["delFlag"] = substr($data["delFlag"],0,100);
		$showRawValues["Destination"] = substr($data["Destination"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "DR Printing";
		$options['firstTime'] = 1;
