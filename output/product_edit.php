...
$templatefile = ($inlineedit == EDIT_INLINE) ? "product_inline_edit.htm" : "product_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["ProdID"] = urldecode(postvalue("editid1"));
$savedKeys["ProdID"] = urldecode(postvalue("editid1"));
...
//	processing Uweight - begin
	...
	if($condition)
	{
		...
		$control_Uweight->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["ProdID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: product_".$pageObject->getPageType().".php?".$keyGetQ);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='product_view.php?".$viewlink."'\"");
