...
$layout->blocks["top"][] = "export";$page_layouts["packinglist_export"] = $layout;
...
$xt->display("packinglist_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=packinglist.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=packinglist.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=packinglist.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["delNo"] = $pageObject->showDBValue("delNo", $row);
			$values["orderNo"] = $pageObject->showDBValue("orderNo", $row);
			$values["delID"] = $pageObject->showDBValue("delID", $row);
			$values["qty"] = $pageObject->showDBValue("qty", $row);
			$values["unit"] = $pageObject->showDBValue("unit", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=packinglist.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["delNo"] = $pageObject->getViewControl("delNo")->showDBValue($row, "");
			$values["orderNo"] = $pageObject->getViewControl("orderNo")->showDBValue($row, "");
			$values["delID"] = $pageObject->getViewControl("delID")->showDBValue($row, "");
			$values["qty"] = $pageObject->getViewControl("qty")->showDBValue($row, "");
			$values["unit"] = $pageObject->getViewControl("unit")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["delNo"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["orderNo"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["delID"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["qty"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["unit"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Del No").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Order No").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Package Details").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Qty").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Unit").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["delNo"]);
					else
						echo $values["delNo"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["orderNo"]);
					else
						echo $values["orderNo"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["delID"]);
				else
					echo $values["delID"];//echo htmlspecialchars($values["delID"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["unit"]);
					else
						echo $values["unit"];
			echo '</td>';
							echo '<td>';
