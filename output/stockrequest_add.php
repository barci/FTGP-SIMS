...
add_nocache_headers();
include("include/stockrequest_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["stockrequest_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "stockrequest_inline_add.htm";
else
	$templatefile = "stockrequest_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: stockrequest_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'stockrequest_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'stockrequest_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: stockrequest_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "stockrequest";
		$options['firstTime'] = 1;
