...
$layout->blocks["top"][] = "search";$page_layouts["Product_List_View_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Product_List_View_search.htm";
...
$params['shortTableName'] = 'Product_List_View';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("CatName"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("CatName")] = GetTableURL($pageObject->pSet->getLookupTable("CatName"));
	
	$pageObject->fillFieldToolTips("CatName");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("CatName");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "CatName";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("CatName_label","<label for=\"".GetInputElementId("CatName", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("CatName_label", true);
	
	$xt->assign("CatName_fieldblock", true);
	$xt->assignbyref("CatName_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("CatName_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("CatName_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_CatName", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("CatName");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CatName", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CatName", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
