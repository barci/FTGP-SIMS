...
add_nocache_headers();
include("include/Gate_Pass_Printing_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Gate_Pass_Printing_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Gate_Pass_Printing_inline_add.htm";
else
	$templatefile = "Gate_Pass_Printing_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Gate_Pass_Printing_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Gate_Pass_Printing_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Gate_Pass_Printing_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Gate_Pass_Printing_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Gate Pass Printing";
		$options['firstTime'] = 1;
