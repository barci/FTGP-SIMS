...
$layout->blocks["top"][] = "details";$page_layouts["orderdetail_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "orderdetail_inline_add.htm";
else
	$templatefile = "orderdetail_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: orderdetail_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="orderentry")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'orderdetail_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'orderdetail_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: orderdetail_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["ODetailID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["ODetailID"]=postvalue("editid1");
	}
