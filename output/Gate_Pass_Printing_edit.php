...
$layout->blocks["top"][] = "details";$page_layouts["Gate_Pass_Printing_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Gate_Pass_Printing_inline_edit.htm" : "Gate_Pass_Printing_edit.htm";
...
//	processing USP - end
//	processing subtotal - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_subtotal = $pageObject->getControl("subtotal", $id);
		$control_subtotal->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing delFlag - begin
	...
	if($condition)
	{
		...
		$control_delFlag->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing delFlag - end
//	processing Destination - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Destination = $pageObject->getControl("Destination", $id);
		$control_Destination->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Gate_Pass_Printing_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Gate_Pass_Printing_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["subtotal"] = $evalues["subtotal"];
	$data["eta"] = $evalues["eta"];
	$data["delFlag"] = $evalues["delFlag"];
	$data["Destination"] = $evalues["Destination"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("prodID_label","<label for=\"".GetInputElementId("prodID", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("delFlag_label","<label for=\"".GetInputElementId("delFlag", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Gate_Pass_Printing_view.php?".$viewlink."'\"");
