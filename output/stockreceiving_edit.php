...
$layout->blocks["top"][] = "details";$page_layouts["stockreceiving_edit"] = $layout;
...
$templatefile = "stockreceiving_edit.htm";
...
//	processing SupplierID - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_SupplierID = $pageObject->getControl("SupplierID", $id);
		$control_SupplierID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: stockreceiving_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: stockreceiving_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["SupplierID"] = $evalues["SupplierID"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("SupplierID_label","<label for=\"".GetInputElementId("SupplierID", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='stockreceiving_view.php?".$viewlink."'\"");
