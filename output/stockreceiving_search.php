...
$layout->blocks["top"][] = "search";$page_layouts["stockreceiving_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "stockreceiving_search.htm";
...
$params['shortTableName'] = 'stockreceiving';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("RequestID"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("RequestID")] = GetTableURL($pageObject->pSet->getLookupTable("RequestID"));
	
	$pageObject->fillFieldToolTips("RequestID");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("RequestID");
	...
	{
		$firstFieldParams['fName'] = "RequestID";
	...
	if(isEnableSection508())
		$xt->assign_section("RequestID_label","<label for=\"".GetInputElementId("RequestID", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("RequestID_label", true);
	
	$xt->assign("RequestID_fieldblock", true);
	$xt->assignbyref("RequestID_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("RequestID_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("RequestID_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_RequestID", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("RequestID");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"RequestID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"RequestID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
