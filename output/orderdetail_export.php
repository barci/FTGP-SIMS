...
$layout->blocks["top"][] = "export";$page_layouts["orderdetail_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["ODetailID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys = array();
			$keys["ODetailID"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("orderdetail_export.htm");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=orderdetail.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=orderdetail.xml");
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=orderdetail.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ODetailID"]).'"';
			if($outstr!="")
				$outstr.=",";
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["ProductID"];//echo htmlspecialchars($values["ProductID"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["BillNo"]);
					else
						echo $values["BillNo"];
			echo '</td>';
			echo "</tr>";
